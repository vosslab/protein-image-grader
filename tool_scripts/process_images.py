...
#============================================
def generate_output_filename(student_entry: dict, filename: str) -> str:
	"""Generate a sanitized output filename for the student's image."""
	clib = commonlib.CommonLib()
	filename = filename.lower()
	basename = os.path.splitext(filename.lowr())[0]
...
#============================================
def read_and_save_student_images(student_tree: list) -> None:
	...
	for student_entry in student_tree:
		...
		output_filename = generate_output_filename(student_entry, image_dict['filename'])
...
def pre_process_student_images(student_tree: list) -> None:
	...
	read_and_save_student_images(student_tree)

	console.print("\nChecking Student Images for Matches", style=data_color)
	check_duplicate_images(student_tree)
...
#============================================
def check_duplicate_images(student_tree: list):
	...
	for student_entry in student_tree:
		...
		if len(local_image_hashes.get(md5hash, [])) > 1:
			...
			student_names = set()
			image_filenames = set()
			for dup_student in local_image_hashes['md5'].get(md5hash):
				student_id_protein.print_student_info(dup_student)
				# Mark S.
				student_name = dup_student['First Name']+" "+dup_student['Last Name'][1]"."
				student_names.add(student_name)
				image_filenames.add(dup_student['output_filename'])
			# just case there are other matches
			for dup_student in local_image_hashes['phash'].get(phash):
				student_id_protein.print_student_info(dup_student)
				# Mark S.
				student_name = dup_student['First Name']+" "+dup_student['Last Name'][1]"."
				student_names.add(student_name)
