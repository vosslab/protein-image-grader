...
#============================================
def find_service_key_file(filename: str = "service_key.json") -> str:
	"""
	Find the service account key file with a small search path.

	Args:
		filename (str): Expected key filename.
	...
	candidates.append(os.path.join(os.path.dirname(cwd), filename))
	candidates.append(os.path.join(home_dir, filename))
	candidates.append(os.path.join(home_dir, "Documents", filename))
	candidates.append(os.path.join(script_dir, filename))

	for entry in os.listdir(cwd):
		entry_path = os.path.join(cwd, entry)
		if os.path.isdir(entry_path):
			candidates.append(os.path.join(entry_path, filename))
	...
	for path in candidates:
		...
		if os.path.isfile(norm_path):
			return norm_path
		tried.append(norm_path)

	message = "service_key.json not found. Tried:\n"
	message += "\n".join(tried)
